...
make_spiral(Ask, St) when is_atom(Ask) ->
    Qs = [
        {label_column, [
            ...
            {?__(3,"Slices"), {text, 16,[{range,{3,128}}]}},
            {?__(4,"Sections"), {text, 16,[{range,{4,128}}]}}
        ]},
...
spiral_faces(Ns0, Nl, L) ->
    ...
    F1 = [lists:seq((Nl-1)*Ns, 0, -Ns) | F0],
    F = [lists:seq(Ns-1, Ns*Nl-1, Ns) | F1],
    F. 
...
spiral_circle2(N, Y, R, D, L) ->
    Delta= pi()*2 / N,
    [{R*cos(I*Delta), Y+I*D, R*sin(I*Delta)} || I <- lists:seq(0, L*N)].
